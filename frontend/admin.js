// ZenTrack Admin Panel JavaScript (Refined for Frontend Deployment)
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const savedToken = localStorage.getItem('zentrack_token');
    ...
    if (savedToken && userStr) {
        ...
        // Redir to login if no token found
        window.location.href = 'login.html';
    }
});
...
// Note: Login is handled by the main login.html
// This logout function acts on the admin panel
function logout() {
    ...
    try {
        ...
        if (response.ok) {
            document.getElementById('statsGrid').innerHTML = `
                ...
                <div class="stat-card">
                    <h3>${data.total_chats || 0}</h3>
                    <p>Total Chats</p>
                </div>
                <div class="stat-card">
                    <h3>${data.total_therapies || 0}</h3>
                    <p>Active Therapies</p>
                </div>
                <div class="stat-card">
                    <h3>${data.total_music || 0}</h3>
                    <p>Music Tracks</p>
                </div>
                <div class="stat-card">
                    <h3>${data.assessed_users || 0}</h3>
                    <p>Assessed Users</p>
                </div>
                <div class="stat-card">
                    <h3>${data.assessment_rate || 0}%</h3>
                    <p>Assessment Rate</p>
                </div>
            `;
        }
...
async function loadTherapies() {
    try {
        const response = await fetch(`${API_BASE_URL}/admin/therapies`, {
            headers: { 'Authorization': `Bearer ${adminToken}` }
        });
...
document.getElementById('therapyForm').addEventListener('submit', async (e) => {
    ...
    try {
        let url = `${API_BASE_URL}/admin/therapies`;
        let method = 'POST';

        if (currentEditId) {
            url = `${API_BASE_URL}/admin/therapies/${currentEditId}`;
            method = 'PUT';
        }
...
async function deleteTherapy(therapyId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/admin/therapies/${therapyId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${adminToken}` }
        });
...
document.getElementById('musicForm').addEventListener('submit', async (e) => {
    ...
    try {
        let url = `${API_BASE_URL}/admin/music`;
        let method = 'POST';

        if (currentEditId) {
            url = `${API_BASE_URL}/admin/music/${currentEditId}`;
            method = 'PUT';
        }
...
async function deleteMusic(musicId) {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/admin/music/${musicId}`, {
            method: 'DELETE',
            headers: { 'Authorization': `Bearer ${adminToken}` }
        });
